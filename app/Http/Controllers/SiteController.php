...
class SiteController extends Controller
{
    function index()
    {
        ...
        return view('front.index', compact('courses', 'c_count', 's_count', 't_count'));
    }
    function course($id)
    {
        $course = Course::find($id);
        $courses = Course::latest('id')->limit(3)->get();
        return view('front.course', compact('course', 'courses'));
    }
    function teacher($id)
    {
        ...
        $event_j = json_encode($events);
        return view('front.teacher', compact('teacher', 'event_j'));
    }
    function search(Request $request)
    {
        $courses = Course::where('name_en', 'like', '%' . request()->q . '%')
            ...
            ->get();
        // $teacher=Teacher::where('name_'.app()->currentLocale(),'like','%'.request()->q.'%')->get();
        dd($courses);
    }
    function enroll($id)
    {
    ...
    function payment(Request $request,$id)
    {
        ...
        if ($code == '000.100.110') {
            ...
            //register student in teacher course
            $course->students()->attach([Auth::id()]);
            return view('front.payment_success')->with('msg', 'Payment Process Successfully');
        } else {
            return redirect()->back()->with('msg', 'Payment Process Failed');
        }
    }
}
